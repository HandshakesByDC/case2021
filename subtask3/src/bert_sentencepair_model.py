...
class BERTSentencePair():

    def __init__(self, model_params_path=None, dataset_name='book_corpus_wiki_en_uncased',
                 bert_size='bert_12_768_12', verbose=True):

        # Mapping
        if bert_size in [12, '12', 'small']:
            bert_size = 'bert_12_768_12'
        if bert_size in [24, '24', 'large']:
            bert_size = 'bert_24_1024_16'
        if dataset_name.lower() in ['english']:
            dataset_name = 'book_corpus_wiki_en_uncased'
        if dataset_name.lower() in ['others', 'multilingual', 'foreign']:
            dataset_name = 'wiki_multilingual_uncased'

        # 1. BERT Pre-Trained Model
        bert_base, vocabulary = nlp.model.get_model(bert_size,
        ...
        if isinstance(model_params_path, str):
            bert_classifier.load_parameters(model_params_path, ctx=ctx)

        self.classifier = bert_classifier
        self.classifier_to_evaluate = bert_classifier
        self.vocabulary = vocabulary
        self.tokenizer = nlp.data.BERTTokenizer(self.vocabulary, lower=True)
        self.dataset_name = dataset_name
        self.bert_size = bert_size
        self.loaded_model_params_path = model_params_path

        vars_attributes = {k:v for k, v in vars(self).items() if k not in ['classifier', 'classifier_to_evaluate', 'tokenizer']}
        if verbose:
            print('Model loaded. Model attributes:')
            pprint(vars_attributes)
            print()
    ...
    def fit(self, data, save_model_as, col_id='id', shuffle=True, val_size=0.2,
            log_interval=4, num_epochs=3, optimize_metric='f0.6'):
        """
    ...
    def train(self, train_data, val_data, log_interval=4, num_epochs=3,
              save_model_as='bert_sentencepair', optimize_metric='f0.6'):
        ...
        for epoch_id in range(num_epochs):
            ...
            val_overall_metrics = self.evaluate_pairwise(val_data)

            print(f'[Epoch {epoch_id + 1} End of Batch], validation_f0.6={val_overall_metrics["f0.6"]:.3f}, validation_recall={val_overall_metrics["recall"]:.3f}, validation_accuracy={val_overall_metrics["accuracy"]:.3f}')
            print(val_overall_metrics)

            # Save Validation Metrics for Epoch
            self.val_overall_metrics.update({f'epoch {epoch_id + 1}': val_overall_metrics})
            ...
            if val_overall_metrics[optimize_metric] > best_metrics[optimize_metric]:

                # Update Best Metrics
                best_metrics = val_overall_metrics
                best_model = model
                # best_val_loss = val_loss_epoch

                # Update Best Model
                self.best_metrics = val_overall_metrics
                self.best_model_params_path = model_params_path # Best Model Parameters
                self.classifier = best_model  # To-do: Unable to deep copy ... Fix it ><
                # self.classifier = model
                # self.classifier = BERTSentencePair(model_params_path=self.best_model_params_path,
                #                                    dataset_name=self.dataset_name).classifier
                print(f'• New best score {optimize_metric}: {val_overall_metrics[optimize_metric]}')

            print()

        # Best Model
        self.classifier_to_evaluate = self.classifier
        print(f'\nOverall best model parameters: {self.best_model_params_path}')

    def predict(self, data):
        """
        Given Data, return cluster predictions.

        Acceptable inputs for Data (data):

            (1) .json file path to Data, where each document should be a dictionary with keys:
                • 'sentence_no'
                • 'sentences'
                • 'id'
            ...
            (3) DataFrame (pandas.DataFrame) comprising columns:
                • 'sentence_no'
                • 'sentences'
                • 'id'
        """
        ...
        if isinstance(data, str):
            data = read(data) # .json file to list of dict
        elif isinstance(data, pd.DataFrame):
            data = data.to_dict('records') # pd.DataFrame to list of dict
    ...
    def evaluate_pairwise(self, val_data):
        ...
        if isinstance(val_data, list):
            val_data = extract_pairwise_sentence_relationships(val_data) # list of dict to pd.DataFrame (pairwise sentences)
        elif isinstance(val_data, str):
            val_data = read(val_data)
            val_data = extract_pairwise_sentence_relationships(val_data) # list of dict to pd.DataFrame (pairwise sentences)
        ...
        val_overall_metrics = overall_metrics(val_predictions, val_truth)

        return val_overall_metrics
    ...
    def evaluate(self, val_data, only_min_clusters=None):
        ...
        if len(data):
            ...
            # (i) Evaluation - overall metrics
            print('(i) overall metrics\n')
            doc_overall_metrics = self.evaluate_pairwise(data)
            print(doc_overall_metrics)
    ...
    def make_data_loader(self, dataset, is_test_dataset=False, batch_size=32,
        ...
        dataset_filename = f'bert_pairwise_sentences_{dataset_type}.tsv'
...
def overall_metrics(predictions, truth):
    ...
    # Fscore weighted
    fscore_metrics = dict()
    for weight in np.arange(0.5,1,0.1):
        weight = float(format(weight,'.1f'))
        fscore_weighted = ((1 + weight**2) * precision * recall) / ((weight**2 * precision) + recall)
        fscore_weighted = float(format(fscore_weighted,'.3f'))
        fscore_metrics[f'f{str(weight)}'] = fscore_weighted
...
def extract_pairwise_sentence_relationships(documents,
                                            ...
                                            col_sentences='sentences', col_id='id'):
    """
    Given Document Data with ground truth, return DataFrame (pandas.DataFrame) comprising columns:
        • 'id' - int of document id
        • 'sentence_1' - string of first sentence
        • 'sentence_2' - string of second sentence
        • 'is_coreferential' - boolean of whether the two sentences are coreferential (ground truth)

    Acceptable inputs for Document Data (documents):

        (1) .json file path to Document Data, where each document should be a dictionary with keys:
            • 'event_clusters'
            • 'sentence_no'
            • 'sentences'
            • 'id'
